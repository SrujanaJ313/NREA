...
function Schedule({ onCancel, selectedRow, event, setSelectedRow }) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const payload = {
          eventId: caseManagerAvl.toString(),
  ...
  useEffect(() => {
    async function fetchCaseManagerAvailibility() {
      try {
        ...
        const newErrMsgs = getMsgsFromErrorCode(
          `GET:${process.env.REACT_APP_CASE_SCHEDULE_CASE_MANAGER_AVL_LIST}`,
          errorResponse
        );
        setErrors(newErrMsgs);
      }
    ...
    async function fetchCaseOfficeName() {
      try {
        const data = await client.get(
          `${scheduleLocalOfficeURL}${selectedRow.claimId}`
        );
        setCaseOfficeName(data);
      } catch (errorResponse) {
        const newErrMsgs = getMsgsFromErrorCode(
          `GET:${process.env.REACT_APP_CASE_SCHEDULE_LOCAL_OFFICE}`,
          errorResponse
        );
        setErrors(newErrMsgs);
      }
        ...
        <Stack direction={"column"} spacing={2}>
          ...
          <Stack>
            <FormControl sx={{ display: "flex", flexDirection: "row" }}>
              ...
              >
                (Choose all that apply. If both boxes are selected, the meeting
                mode for this appointment will default to In person.)
              </Typography>
            </FormControl>
